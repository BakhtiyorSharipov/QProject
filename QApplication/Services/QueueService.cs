...
public class QueueService : IQueueService
{
    ...
    public async Task<IEnumerable<QueueResponseModel>> GetAllAsync(int pageList, int pageNumber)
    {
        _logger.LogInformation("Getting all queues. PageNumber: {pageNumber}, PageList: {pageList}", pageNumber,
            pageList);
        var dbQueue = await _repository.GetAll(pageList, pageNumber).ToListAsync();
        if (dbQueue == null)
        {
    ...
    public async Task<QueueResponseModel> GetByIdAsync(int id)
    {
        _logger.LogInformation("Getting queue by Id {id}", id);
        var dbQueue = await _repository.FindByIdAsync(id);
        if (dbQueue == null)
        {
    ...
    public async Task<AddQueueResponseModel> AddAsync(QueueRequestModel requestModel)
    {
        ...
        var schedule = await _scheduleRepository.GetEmployeeById(requestToCreate.EmployeeId).ToListAsync();
        if (!schedule.Any())
        {
        ...
        var customer = await _customerRepository.FindByIdAsync(requestToCreate.CustomerId);
        if (customer == null)
        {
        ...
        var service = await _serviceRepository.FindByIdAsync(requestToCreate.ServiceId);
        if (service == null)
        {
        ...
        if (!slotExists)
        {
            _logger.LogWarning("Time slot {startTime} not available for EmployeeId: {id}", requestToCreate.StartTime,
                requestToCreate.EmployeeId);
            throw new Exception(
        ...
        var allQueuesByEmployee = await GetQueuesByEmployeeAsync(requestToCreate.EmployeeId);
        ...
        var blocked = await _blockedCustomerRepository.FindByIdAsync(requestToCreate.CustomerId);
        if (blocked != null &&
            blocked.DoesBanForever &&
            service.CompanyId == blocked.CompanyId)
        {
            _logger.LogWarning("Customer {id} is blocked from Company {companyId}", requestToCreate.CustomerId,
                service.CompanyId);
    ...
    public async Task<QueueResponseModel> CancelQueueByCustomerAsync(QueueCancelRequest request)
    {
        _logger.LogInformation("Cancelling queue Id {id} by customer", request.QueueId);
        var dbQueue = await _repository.FindByIdAsync(request.QueueId);
        if (dbQueue == null)
        {
    ...
    public async Task<QueueResponseModel> CancelQueueByEmployeeAsync(QueueCancelRequest request)
    {
    ...
    public async Task<UpdateQueueStatusResponseModel> UpdateQueueStatusAsync(UpdateQueueRequest request)
    {
        ...
        _logger.LogDebug("Current queue status: {CurrentStatus}, requested new status: {NewStatus}", dbQueue.Status,
            request.newStatus);
        var employeeSchedule = await _scheduleRepository.GetEmployeeById(dbQueue.EmployeeId).ToListAsync();
        ...
        if (request.newStatus == QueueStatus.DidNotCome)
        {
            ...
            var queuesByCustomer = await _repository.GetQueuesByCustomer(dbQueue.CustomerId).ToListAsync();
            ...
            if (count >= 3 && !_blockedCustomerRepository.Exists(dbQueue.CustomerId, dbQueue.Service.CompanyId))
            {
                ...
                await _blockedCustomerRepository.SaveChangesAsync();
        ...
        if (request.newStatus == QueueStatus.Confirmed)
        {
            ...
            if (request.EndTime.HasValue)
            {
                ...
                var allQueueByEmployee = await _repository.GetQueuesByEmployee(dbQueue.EmployeeId)
    ...
    public async Task<IEnumerable<QueueResponseModel>> GetQueuesByCustomerAsync(int customerId)
    {
        _logger.LogInformation("Getting queues for CustomerId: {CustomerId}", customerId);
        var dbQueue = await _repository.GetQueuesByCustomer(customerId).ToListAsync();
        if (!dbQueue.Any())
        {
    ...
    public async Task<IEnumerable<QueueResponseModel>> GetQueuesByEmployeeAsync(int employeeId)
    {
        _logger.LogInformation("Getting queues for EmployeeId: {EmployeeId}", employeeId);
        var dbQueue = await _repository.GetQueuesByEmployee(employeeId).ToListAsync();
        if (!dbQueue.Any())
        {
    ...
    public async Task<IEnumerable<QueueResponseModel>> GetQueuesByServiceAsync(int serviceId)
    {
        _logger.LogInformation("Getting queues for ServiceId: {ServiceId}", serviceId);
        var dbQueue = await _repository.GetQueuesByService(serviceId).ToListAsync();
        if (!dbQueue.Any())
        {
        ...
        _logger.LogInformation("Successfully fetched {QueueCount} queues for ServiceId: {ServiceId}", response.Count,
            serviceId);
        return response;
    }

    public async Task<IEnumerable<QueueResponseModel>> GetQueuesByCompanyAsync(int companyId)
    {
        _logger.LogInformation("Getting queues for CompanyId: {CompanyId}", companyId);
        var dbQueue = await _repository.GetQueuesByCompany(companyId).ToListAsync();
        if (!dbQueue.Any())
        {
        ...
        _logger.LogInformation("Successfully fetched {QueueCount} queues for CompanyId: {CompanyId}", response.Count,
            companyId);
        return response;
    }
}